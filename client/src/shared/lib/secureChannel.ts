...
export class SecureChannel {
  ...
  static async initNew(): Promise<{ channel: SecureChannel; publicKey: string }> {
    ...
    sc.keyPair = await crypto.subtle.generateKey({ name: "ECDH", namedCurve: "P-256" }, true, [
      "deriveKey",
    ]);
  ...
  static async initWithRemoteKey(
    base64RemoteKey: string
  ): Promise<{ channel: SecureChannel; publicKey: string }> {
    console.log(
      "[CR] SecureChannel.initWithRemoteKey → публичный ключ от собеседника:",
      base64RemoteKey
    );
    const sc = new SecureChannel();
  ...
  async decryptMessage(cipher: string, ivBase64: string): Promise<string> {
    ...
    const decrypted = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, this.sharedSecret, data);
